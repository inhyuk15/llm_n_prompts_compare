...
static void handle_valid_transaction(int32_t money, int32_t prod_num)
{
    ...
    else
    {
        ...
        int32_t change = money - selected->price;
...
static void process_input_line(void)
{
    ...
    if (!extract_tokens(input_buffer, money_str, &money_len, product_str, &product_len))
    {
        input_valid = false;
    }
    else
    {
        ...
        prod_num = parse_int(product_str, product_len);
