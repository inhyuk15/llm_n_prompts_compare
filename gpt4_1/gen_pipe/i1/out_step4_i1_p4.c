...
{
    // Validate pin range (typical ESP32 pins 0 - 39)
...
{
    // Guard against NaN inputs can be considered, but not mandatory here
    switch (op) {
        case OP_ADD:
        ...
        case OP_DIV:
            ...
            return a / b;
        case OP_POW:
            // pow() can return domain error - guard for domain
            if ((a == 0.0) && (b <= 0.0)) return 0.0; // zero base, non-positive exponent undefined
            return pow(a, b);
        default:
            return 0.0;
    }
...
static double calculate_single(const double a, const Operator op)
{
    switch (op) {
        ...
        case OP_LOG:
            ...
            return log10(a);
        case OP_EXP:
            if (a > 700.0) return 0.0; // prevent overflow in exp
            if (a < -700.0) return 0.0; // prevent underflow in exp
            return exp(a);
        default:
            return 0.0;
    }
...
static void reset_calc(void)
{
    ...
    (void)strncpy(calc.display, "0", sizeof(calc.display));
...
static void handle_operator_button(void)
{
    ...
    // Using OP_ADD as a placeholder operator per original code.
