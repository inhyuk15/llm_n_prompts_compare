...
{
    assert(e != NULL);
    /* Ensure current_floor < FLOOR_COUNT to avoid overflow */
    if (e->current_floor < FLOOR_COUNT) {
        assert(e->current_floor + 1 <= FLOOR_COUNT); /* no overflow */
        e->current_floor++;
        e->direction = 1;
    }
}
...
{
    assert(e != NULL);
    /* Ensure current_floor > 1 to avoid underflow */
    if (e->current_floor > 1) {
        assert(e->current_floor - 1 >= 1); /* no underflow */
        e->current_floor--;
        e->direction = -1;
    }
}
...
static void move_elevator_one_step(elevator_t * const e)
{
    ...
    /* Post condition: floor bounds and direction */
    assert(e->current_floor >= 1 && e->current_floor <= FLOOR_COUNT);
    assert(e->direction >= -1 && e->direction <= 1);
}
...
static int32_t find_closest_elevator(int32_t floor)
{
    ...
    /* Ensure result is valid index or -1 */
...
static void lcd_format_line1(char *buf, size_t size)
{
    ...
    /* Validate elevator floors and moving flags */
...
static void process_floor_request(int32_t floor)
{
    ...
    assert(elv >= 0 && elv < (int32_t)ELEVATOR_COUNT);
...
static void button_task(void * arg)
{
    ...
    while (task_continue != pdFALSE) {
        if (gpio_evt_queue == NULL) {
            /* Queue deleted or not initialized, exit */
            break;
        }
...
void app_main(void)
{
    ...
    if (gpio_evt_queue != NULL) {
        ...
        /* Queue creation failed - not handled as per instructions */
