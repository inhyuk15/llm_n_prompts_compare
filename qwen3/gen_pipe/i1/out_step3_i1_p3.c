...
void displayInput() {
  lcd.setCursor(0, 0);
  char line1[MAX_DISPLAY_BUFFER];
  size_t inputLen = strlen(input);
  if (inputLen > LCD_WIDTH) {
    truncateToLCD(line1, input + (inputLen - LCD_WIDTH), LCD_WIDTH);
  } else {
  ...
  char line2[MAX_DISPLAY_BUFFER];
  size_t numLen = strlen(currentNum);
  if (numLen > LCD_WIDTH) {
    truncateToLCD(line2, currentNum + (numLen - LCD_WIDTH), LCD_WIDTH);
  } else {
