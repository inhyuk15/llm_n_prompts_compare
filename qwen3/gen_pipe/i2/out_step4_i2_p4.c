...
void parseInput(const char* buffer) {
  ...
  if (spaceIndex != -1) {
    char *endptr1;
    stop_time = strtof(buffer, &endptr1);
    if (endptr1 == buffer || spaceIndex != (endptr1 - buffer)) return;

    char *endptr2;
    walk_time = strtof(buffer + spaceIndex + 1, &endptr2);
    if (endptr2 == (buffer + spaceIndex + 1) || *endptr2 != '\0') return;

    if (stop_time <= 0.0f || walk_time <= 0.0f) return;
  }
}
...
void loop() {
  ...
  if (walk_time > threshold) {
    float durationBlue = walk_time - threshold;
  ...
  if (stop_time * 1000.0F > (float)UINT32_MAX) return;
  uint32_t stopMillis = (uint32_t)(stop_time * 1000.0F);
