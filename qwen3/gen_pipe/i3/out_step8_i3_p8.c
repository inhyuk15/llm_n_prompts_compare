 ...
 */
static const int product_prices[] = {PRODUCT_PRICE_1, PRODUCT_PRICE_2, PRODUCT_PRICE_3};
...
// Function prototypes
static ErrorCode trim_input(char* input);
static ErrorCode parse_input(const char* input, int* amount, int* product_num);
static ErrorCode validate_product(int product_num);
static ErrorCode acquire_lcd(void);
static ErrorCode release_lcd(void);
static ErrorCode acquire_led(void);
static ErrorCode release_led(void);
static ErrorCode display_invalid_format(void);
static ErrorCode display_invalid_product(void);
static ErrorCode display_not_enough_money(void);
static ErrorCode dispense_product(int amount, int product_num);
...
void loop() {
    if (Serial.available()) {
        ...
        ErrorCode err = trim_input(input_buffer);
        if (err != ERROR_NONE) {
            display_invalid_format();
            return;
        }
        
        int amount = 0, product_num = 0;
        err = parse_input(input_buffer, &amount, &product_num);
        if (err != ERROR_NONE) {
            display_invalid_format();
            return;
        }
        ...
        err = validate_product(product_num);
        if (err != ERROR_NONE) {
            display_invalid_product();
            return;
        }
        
        err = dispense_product(amount, product_num);
 ...
 * @param product_num Pointer to store parsed product number.
 * @return ErrorCode Returns ERROR_NONE on success, or appropriate error code.
 */
static ErrorCode parse_input(const char* input, int* amount, int* product_num) {
    if (input == NULL || amount == NULL || product_num == NULL) {
        return ERROR_NULL_POINTER;
    }

    int parsed = sscanf(input, "%d %d", amount, product_num);
    if (parsed != 2) {
 ...
 *
 * @param product_num Product number to validate.
 ...
 */
static ErrorCode display_invalid_format() {
 ...
 * @return ErrorCode Returns ERROR_NONE on success, or error code from mutex.
 */
static ErrorCode display_invalid_product() {
    ErrorCode err = acquire_lcd();
    if (err != ERROR_NONE) return err;
    lcd.clear();
 ...
 * @return ErrorCode Returns ERROR_NONE on success, or error code from mutex.
 */
static ErrorCode display_not_enough_money() {
    ErrorCode err = acquire_lcd();
    if (err != ERROR_NONE) return err;
    lcd.clear();
 ...
 * @param product_num Selected product number.
 * @return ErrorCode Returns ERROR_NONE on success, or appropriate error code.
 */
static ErrorCode dispense_product(int amount, int product_num) {
    if (product_num < 1 || product_num > PRODUCT_COUNT) {
    ...
    int price_index = product_num - 1;
    int price = product_prices[price_index];
